defmodule Templatelib.Lexer do
  ...
  # NEXT RETURN
  defp next(input, next_type, _, [%Token{type: :eof, literal: _} | _] = tokens, rest, inside)
       when is_next_type(next_type) and is_boolean(inside) do
    returned_rest =
      case next_type do
        :peek -> input
        :pop -> rest
  ...
  # LEX CALC
  @spec lex(input :: String.t(), tokens :: [Token.t()], inside_delimiter :: boolean()) ::
          {Token.t(), String.t(), boolean()}
  # ANY POINT
  # EOF CASE
  defp lex(<<>>, _tokens, _inside), do: {Token.new(:eof), <<>>, false}
  # OUTSIDE DELIMITER
  defp lex(input, _tokens, false) when is_binary(input), do: read_until_open(input, <<>>)
  # INSIDE DELIMITER
  # WHITESPACES
  defp lex(<<c::8, rest::binary>>, tokens, true) when is_whitespace(c),
    do: lex(rest, tokens, true)

  # TOKENIZATION
  defp lex(input, _tokens, true), do: tokenize(input)
  ...
  defp tokenize(<<"=", rest::binary>>), do: {Token.new(:equal), rest, true}
  ...
  defp tokenize_word("get"), do: Token.new(:get)
  defp tokenize_word("as_string"), do: Token.new(:as_string)
  defp tokenize_word("padding"), do: Token.new(:padding)
